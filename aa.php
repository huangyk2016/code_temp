...
class ipChk {
    ...
    function checkFjIp($srcIp) {
        $addr = $this->ip2addr($srcIp);

        if(false === strpos($addr['country'], '福建')) {
            echo "outside<br/>";
            return false;
        }
        else {
    ...
    $addr = $ipCheck -> ip2addr($strIp);
    print_r($addr);
    echo " <br/>------".$addr['country']."--------<br/>";
    //$curl_res = mb_convert_encoding($addr['country'], 'gbk', 'utf-8');
    $pos = mb_strpos($addr['country'], '福建');
    echo $pos;
    echo '<br/>';
    var_dump($pos);
    echo " <br/>--------------<br/>";
